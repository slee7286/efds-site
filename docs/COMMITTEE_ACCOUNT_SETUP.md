...
3. **Finish from your inbox.** Open the newest EFDS sign-in email on the same device and browser where you started. Follow its link, set and confirm a password if prompted, and continue to the dashboard. Email links expire and work only once; request a fresh one if needed. Check junk mail if no email arrives.

...
1. In [Google Auth Platform](https://console.cloud.google.com/auth), select a Google Cloud project controlled by EFDS. Set the app name and support email in **Branding**. Under **Audience**, choose **External** unless an Imperial-owned Google Workspace project intentionally restricts the app to that Workspace; an Internal app cannot serve Google accounts outside its organization. If Google asks for test users while the app is in Testing, add the designated account for the first check. Request only the identity scopes `openid`, `userinfo.email` and `userinfo.profile`; Gmail, Drive and Calendar access are not needed for login.
...
5. Open `https://www.imperial-efds.com/login` in a private browser session. The **Continue with Google** button should appear after the provider-settings cache refreshes (up to five minutes). First use an eligible Imperial address that already has an EFDS account: confirm arrival at `/dashboard`, the same profile and role, and a persistent session after reload. Then test an eligible new account, an ineligible address, logout and email/password sign-in. If Google creates a second EFDS identity for an existing address, stop and investigate linking before promoting either account. Do not share the Google client secret or session tokens in chat.

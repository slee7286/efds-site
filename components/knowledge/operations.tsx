...
export function CompactReviewActions({ item }: { item: KnowledgeReviewItem }) {
  return <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
    <form action={reviewKnowledgeAction}><HiddenFields item={item} action="approve" /><SubmitButton className="button button-primary" type="submit" title="Approve"><Check size={13} /> Approve</SubmitButton></form>
    <form action={reviewKnowledgeAction}><HiddenFields item={item} action="needs_review" /><SubmitButton className="button button-quiet" type="submit" title="Defer"><History size={13} /> Defer</SubmitButton></form>
  </div>;
}
...
export function ReviewDetailActions({ item }: { item: KnowledgeReviewItem }) {
  return <div className="surface info-card" style={{ marginTop: 18 }}><div className="panel-heading"><h2>Review action</h2><span className="muted">Admin only</span></div><div style={{ display: "grid", gap: 16 }}>
    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}><form action={reviewKnowledgeAction}><HiddenFields item={item} action="approve" /><SubmitButton className="button button-primary" type="submit"><Check size={14} /> Approve current interpretation</SubmitButton></form><form action={reviewKnowledgeAction}><HiddenFields item={item} action="needs_review" /><SubmitButton className="button button-quiet" type="submit"><History size={14} /> Needs review</SubmitButton></form><form action={reviewKnowledgeAction}><HiddenFields item={item} action="supersede" /><SubmitButton className="button button-quiet" type="submit"><RotateCcw size={14} /> Supersede</SubmitButton></form></div>
    <form action={reviewKnowledgeAction} style={{ display: "grid", gap: 8 }}><HiddenFields item={item} action="edit_approve" /><label htmlFor={`edit-${item.id}`}><strong>Edit and approve</strong></label><textarea id={`edit-${item.id}`} name="normalizedText" defaultValue={item.normalizedText} rows={4} required /><SubmitButton className="button button-dark" type="submit"><Check size={14} /> Save interpretation and approve</SubmitButton></form>
    <form action={reviewKnowledgeAction} style={{ display: "grid", gap: 8 }}><HiddenFields item={item} action="reject" /><label htmlFor={`reason-${item.id}`}><strong>Reject with reason</strong></label><select className="select" name="reason" id={`reason-${item.id}`} defaultValue="false positive" required><option value="false positive">False positive</option><option value="not applicable to EFDS">Not applicable to EFDS</option><option value="duplicate">Duplicate</option><option value="outdated">Outdated</option><option value="descriptive rather than mandatory">Descriptive rather than mandatory</option><option value="incorrectly interpreted">Incorrectly interpreted</option><option value="insufficient evidence">Insufficient evidence</option><option value="other">Other</option></select><SubmitButton className="button button-outline" type="submit"><X size={14} /> Reject</SubmitButton></form>
  </div></div>;
}

export function PublicationControls({ item }: { item: KnowledgeReviewItem }) {
  return <div className="surface info-card" style={{ marginTop: 18 }}><div className="panel-heading"><h2>Publication</h2><span className="muted">Explicit selection</span></div><p className="muted">Only approved, current knowledge can leave the internal boundary. Publication never changes the source article.</p><div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}><form action={reviewKnowledgeAction}><HiddenFields item={item} action="publish" /><select className="select" name="visibility" defaultValue={item.visibility} aria-label="Visibility"><option value="internal">Internal</option><option value="committee">Committee</option><option value="member">Member</option><option value="public">Public</option></select><SubmitButton className="button button-primary" type="submit"><Check size={14} /> Set visibility</SubmitButton></form><form action={reviewKnowledgeAction}><HiddenFields item={item} action="publish" /><input type="hidden" name="visibility" value="internal" /><SubmitButton className="button button-quiet" type="submit">Keep internal</SubmitButton></form></div></div>;
}

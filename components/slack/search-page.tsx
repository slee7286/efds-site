...
import { SlackChannelTable, SlackNav, SlackSearchForm, type SlackArchiveBasePath } from "@/components/slack/archive";
import { listSlackAuthors, listSlackChannels, listSlackMessages } from "@/lib/db/slack";
import type { SlackMessage } from "@/types/domain";

export type SlackSearchParams = { q?: string; channel?: string; author?: string; from?: string; to?: string; edited?: string; deleted?: string; page?: string };

export async function SlackArchiveSearchPage({ searchParams, basePath }: { searchParams?: Promise<SlackSearchParams>; basePath: SlackArchiveBasePath }) {
  ...
  return <div className="app-content">
    <div className="eyebrow">{basePath === "/admin/slack" ? "Admin" : "Committee"} · Slack archive</div>
    ...
    <SlackNav basePath={basePath} />
    <SlackSearchForm query={query.q} channel={query.channel} author={query.author} from={query.from} to={query.to} edited={query.edited === "true"} deleted={query.deleted === "true"} channels={channels} authors={authors} basePath={basePath} />
    <section aria-label="Archived Slack message results" style={{ marginBottom: 28 }}>
      ...
      {results.items.map((message) => <SlackMessageResult key={message.id} message={message} basePath={basePath} />)}
    ...
    <SlackChannelTable channels={channels} basePath={basePath} />
  </div>;
}

function SlackMessageResult({ message, basePath }: { message: SlackMessage; basePath: SlackArchiveBasePath }) {
  ...
  return <Link className="queue-row" href={`${basePath}/messages/${message.id}`}>

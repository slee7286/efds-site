...
import { ArrowUpRight, BookOpen, BriefcaseBusiness, CalendarDays, CircleHelp, LayoutDashboard, ListTodo, Menu, Search, Settings, UserRound, UsersRound, X } from "lucide-react";
...
export function AppSidebar({ role = "member", onNavigate }: { role?: AccessRole; onNavigate?: () => void }) {
  ...
  const hasSocietyOperations = role === "committee" || role === "admin";
  const items = memberItems.filter(([, href]) => (href !== "/dashboard/knowledge" || role !== "viewer") && (!hasSocietyOperations || href !== "/dashboard"));
  const operationsItems = hasSocietyOperations ? [["Overview", "/dashboard", LayoutDashboard], ["Tickets", "/dashboard/tickets", ListTodo], ...(role === "admin" ? adminItems : committeeItems)] as const : [];
  return <aside className="app-sidebar" onClick={(event) => { if ((event.target as HTMLElement).closest("a")) onNavigate?.(); }}><Brand compact />{hasSocietyOperations && <><div className="sidebar-label">Society operations</div><nav className="sidebar-nav" aria-label="Society operations navigation">{operationsItems.map(([label,href,Icon]) => <Link key={href} className={active(href) ? "active" : undefined} aria-current={active(href) ? "page" : undefined} href={href}><Icon size={16} />{label}</Link>)}</nav></>}<div className="sidebar-label">Your workspace</div><nav className="sidebar-nav" aria-label="Private navigation">{items.map(([label,href,Icon]) => <Link key={href} className={active(href) ? "active" : undefined} aria-current={active(href) ? "page" : undefined} href={href}><Icon size={16} />{label}</Link>)}</nav><div className="sidebar-bottom sidebar-nav"><Link href="/"><ArrowUpRight size={16} />Back to public site</Link></div></aside>;
}

...
export function MeetingTable({ items }: { items: MeetingArchiveItem[] }) {
  return <div className="surface" style={{ overflowX: "auto" }}><table className="ops-table"><thead><tr><th>Meeting</th><th>Date</th><th>Transcript</th><th>Summary</th><th>Changed</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><Link href={`/admin/meetings/${item.id}`}><strong>{item.title}</strong></Link><small>{item.externalMeetingId || item.sourceType}</small></td><td>{date(item.startedAt)}</td><td>{item.transcriptAvailable ? <span className="badge badge-mint">available</span> : <span className="badge badge-neutral">missing</span>}</td><td>{item.summaryAvailable ? <span className="badge badge-mint">available</span> : <span className="badge badge-neutral">missing</span>}</td><td>{date(item.lastChangedAt || item.sourceUpdatedAt)}</td></tr>)}</tbody></table>{!items.length && <div className="empty-state"><CalendarDays size={22} /><h2>No Meetily meetings found.</h2><p>Run the backend Meetily synchronizer after applying its migration.</p></div>}</div>;
}
...
export function MeetingFacts({ item, metadata }: { item: MeetingArchiveItem; metadata: Record<string, unknown> }) {
  return <dl className="security-details"><div><dt>When</dt><dd>{date(item.startedAt)}</dd></div><div><dt>Duration</dt><dd>{duration(item.durationSeconds)}</dd></div><div><dt>Meetily ID</dt><dd><code>{item.externalMeetingId || "—"}</code></dd></div><div><dt>Source</dt><dd>{item.sourceType}</dd></div><div><dt>Source reference</dt><dd>{typeof metadata.source_database === "string" ? metadata.source_database : typeof metadata.source_export === "string" ? metadata.source_export : "—"}</dd></div><div><dt>Last seen</dt><dd>{date(item.lastSeenAt)}</dd></div></dl>;
}

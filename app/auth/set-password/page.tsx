...
export default async function SetPasswordPage({ searchParams }: { searchParams: Promise<{ flow?: string }> }) {
  ...
  return <main className="auth-page" id="main-content"><section className="auth-aside"><Brand /><div><div className="eyebrow">Secure account access</div><h1 className="display">Your account,<br /><em>your key.</em></h1><p>Set a password for your approved EFDS email account. Your password is handled securely by our sign-in service.</p></div><Link className="button button-quiet" style={{ color: "rgba(247,247,243,.7)" }} href="/login"><ArrowLeft size={14} /> Back to sign in</Link></section><section className="auth-panel"><PasswordForm mode={mode} /></section></main>;
}

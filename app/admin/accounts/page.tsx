  ...
  return <div className="app-content account-review-page">
    ...
    <section className="account-review-list" aria-label="Accounts">
      {data.accounts.map((account) => <article className="surface account-review-card" key={account.id}>
        <div className="account-review-identity"><div><span className="eyebrow">{account.memberType} account</span><h2>{account.fullName || account.email}</h2><p>{account.email}</p></div><div className="account-review-badges"><span className="badge badge-neutral">{roleLabel(account.role)}</span><span className={`badge ${account.verificationStatus === "approved" ? "badge-mint" : "badge-neutral"}`}>{account.verificationStatus}</span></div></div>
      ...
      {!data.accounts.length && <div className="surface empty-state"><h2>{status === "pending" ? "No accounts need review." : "No active accounts found."}</h2><p>{status === "pending" ? "New accounts will appear here after their first sign-in." : "Accounts appear after people complete sign-in."}</p></div>}
    </section>

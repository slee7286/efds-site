...
export default async function AdminKnowledgePage() {
  ...
  return <div className="app-content">
    ...
    <div className="app-grid" style={{ marginTop: 28 }}><section className="surface info-card"><div className="panel-heading"><h2>Review status</h2><Link href="/admin/knowledge/review">Open queue <ArrowRight size={13} /></Link></div><dl className="security-details"><MetricRow label="Proposed" value={dashboard.review.proposed ?? 0} href="/admin/knowledge/review?status=proposed" /><MetricRow label="Needs review" value={dashboard.review.needs_review ?? 0} href="/admin/knowledge/review?status=needs_review" /><MetricRow label="Approved" value={dashboard.review.approved ?? 0} href="/admin/knowledge/review?status=approved" /><MetricRow label="Rejected" value={dashboard.review.rejected ?? 0} href="/admin/knowledge/review?status=rejected" /><MetricRow label="Superseded" value={dashboard.review.superseded ?? 0} href="/admin/knowledge/review?status=superseded" /></dl></section>
      <aside id="source-health" className="surface-dark info-card"><div className="eyebrow" style={{ color: "var(--mint)" }}>System health</div><div style={{ display: "grid", gap: 18, marginTop: 24 }}><HealthRow icon={<RefreshCw size={15} />} label="Latest ICU sync" value={`${value(dashboard.latestSync, "status") ?? "Not connected"} · ${formatDate(value(dashboard.latestSync, "finished_at"))}`} /><HealthRow icon={<Database size={15} />} label="Latest extraction" value={`${value(dashboard.latestExtraction, "status") ?? "Not run"} · ${formatDate(value(dashboard.latestExtraction, "finished_at"))}`} /><HealthRow icon={dashboard.attention ? <AlertTriangle size={15} color="var(--coral)" /> : <CheckCircle2 size={15} color="var(--mint)" />} label="Attention" value={`${dashboard.attention} record(s)`} /></div></aside>
    </div>
    <p className="muted" style={{ fontSize: 11, marginTop: 16 }}>Counts reflect the connected archive. Local design previews show empty records.</p>
  </div>;
}

...
export default async function SlackChannelsPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  const [channels, authors] = await Promise.all([listSlackChannels(), listSlackAuthors()]);
  const hasSearch = Boolean(query.q || query.channel || query.author || query.from || query.to || query.edited || query.deleted);
  const messages = hasSearch ? await listSlackMessages({ query: query.q, channelId: query.channel, authorId: query.author, from: query.from ? `${query.from}T00:00:00.000Z` : undefined, to: query.to ? `${query.to}T23:59:59.999Z` : undefined, edited: query.edited === "true", deleted: query.deleted === "true" }) : [];

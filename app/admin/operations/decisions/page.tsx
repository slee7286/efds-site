...
export default async function DecisionsPage({ searchParams }: { searchParams?: Promise<Record<string, string | undefined>> }) {
  ...
  const [items, workstreams] = await Promise.all([readPageData(() => listOperationalRecords({ type: "decision", review: params.review, query: params.q, workstream: params.workstream }), []), readPageData(() => getOperationalWorkstreams(), [])]);

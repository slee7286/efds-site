...
export default async function QuestionsPage({ searchParams }: { searchParams?: Promise<Record<string, string | undefined>> }) {
  ...
  const [items, workstreams] = await Promise.all([listOperationalRecords({ type: "open_question", review: params.review, execution: params.execution, query: params.q, workstream: params.workstream }), getOperationalWorkstreams()]);

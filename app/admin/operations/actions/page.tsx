...
export default async function ActionsPage({ searchParams }: { searchParams?: Promise<Record<string, string | undefined>> }) {
  ...
  const [items, workstreams] = await Promise.all([listOperationalRecords({ type: "action_item", review: params.review, execution: params.execution, query: params.q, workstream: params.workstream, overdue: params.overdue === "true" }), getOperationalWorkstreams()]);

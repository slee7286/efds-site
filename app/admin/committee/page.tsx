...
export default async function AdminCommitteePage({ searchParams }: { searchParams: Promise<{ error?: string; notice?: string; mail?: string }> }) {
  ...
  return <div className="app-content committee-admin-page">
    ...
    <div className="committee-directory-bottom">
      ...
      <section className="surface app-panel"><h2>Recent member accounts</h2><p>First sign-in creates an EFDS member account. Verify EFDS degree enrolment, then grant committee access and link the roster entry.</p>{recentMembers.map((profile) => <div className="queue-row" key={profile.email}><div><strong>{profile.fullName || profile.email}</strong><small>{profile.email}</small></div><span className="badge badge-neutral">{profile.role === "efds_member" ? "EFDS student" : "EFDS member"}</span></div>)}{!recentMembers.length && <p className="muted">No member accounts have signed in yet.</p>}</section>
    </div>
  </div>;
}

...
export default async function AdminIntegrationsPage() {
  const email = await getAuthEmailHealth();
  ...
  return <div className="app-content">
    ...
    <section className="surface info-card" aria-labelledby="auth-email-health-heading">
      <div className="panel-heading"><h2 id="auth-email-health-heading">Authentication email</h2><span className={`badge ${attention ? "badge-coral" : "badge-mint"}`}>{attention ? "Check delivery" : "No recorded failures"}</span></div>
    ...
    <section className="surface info-card" style={{ marginTop: 20 }} aria-labelledby="source-status-heading"><h2 id="source-status-heading">Other sources</h2>
      ...
      <div className="queue-row"><div><h3>Outlook</h3><p>Sender-limited ingestion awaits a delegated mailbox connection.</p></div><span className="badge badge-neutral">Not connected</span></div>
    </section>
  </div>;
}

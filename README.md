...
- Public editorial website: home, about, events, careers, research, competitions, resources, partners and public chat
...
- First-class external-user exception contract for future `auth_access_exceptions` rows
...
The backend currently has no application `profiles` or `auth_access_exceptions` model. This website therefore ships the contract, access utilities and safe Supabase query boundary, but no website-owned migration. Add those models and an Alembic migration in `efds-knowledge-base` before production auth is enabled. Do not run that migration automatically from this repository.
...
The primary login action is `Continue with Microsoft`, implemented with Supabase Azure OAuth and minimal identity scopes: `openid profile email`. The callback exchanges the code server-side, obtains the user, normalizes the verified email, checks the Imperial domain policy, checks the external exception contract, upserts an Imperial profile when the backend contract exists, then redirects to `/dashboard` or `/access-denied`.

External users use the secondary Supabase magic-link flow. The UI does not promise access merely because an email can receive a link; the server must find an active, unexpired exception for the normalized address. External exceptions should default to `viewer` or `member`, never `admin` without an explicit trusted decision.
...
6. Add the backend-owned `profiles` and `auth_access_exceptions` models/migration, enable RLS and provision the first admin out-of-band.
...
1. Add `profiles` and `auth_access_exceptions` to the backend owner with RLS.
2. Generate Supabase database types from the deployed schema.
3. Connect public publication views and the real officer/event reads.
4. Add committee review actions with auditability.
5. Add private retrieval with document-level scope enforcement.
6. Add saved careers/jobs persistence, then narrowly scoped integrations.

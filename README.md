...
The ticket panel lets committee members request one idea from recent messages in enabled public Slack channels. The agent uses a dedicated committee-scoped retrieval RPC. The reviewer can open the cited message, edit the title and rationale, choose workstream, priority and assignees, and must explicitly confirm review before publication. The database rechecks the source and role, links the evidence, rejects an exact duplicate open title and records the actor. Admins can also request ideas from private meeting notes or the wider Slack archive; those save as private operational proposals for the existing audited approval flow. Nothing publishes automatically. Recent Outlook mailbox history is not available to this workflow until a mailbox sync and retrieval source are configured. Committee account steps are available as an editable [Markdown source](docs/COMMITTEE_ACCOUNT_SETUP.md) and a [Word guide](docs/EFDS_Committee_Account_Setup_Guide.docx).

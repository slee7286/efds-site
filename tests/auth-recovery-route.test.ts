...
const { exchangeCodeForSession, getUser, signOut, provisionAuthenticatedProfile, evaluateUserAccess, supabase } = vi.hoisted(() => {
  const exchangeCodeForSession = vi.fn();
  const getUser = vi.fn();
  const signOut = vi.fn();
  const provisionAuthenticatedProfile = vi.fn();
  const evaluateUserAccess = vi.fn();
  return { exchangeCodeForSession, getUser, signOut, provisionAuthenticatedProfile, evaluateUserAccess, supabase: { auth: { exchangeCodeForSession, getUser, signOut } } };
});
